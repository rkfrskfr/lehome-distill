...
Isaac Sim 은 필요 없다. 그래픽카드도 필요 없다. 노트북 한 대에 전부 올리는 것이 권장 구성이다.
...
노트북 한 대로 충분한 이유
- 소형 모델은 51.7M 이라 **CPU 로 한 번 계산에 10밀리초**다 (실측). 30Hz 루프에 여유가 크다.
- 모델은 5스텝마다 한 번만 새로 계산하므로 실제 부하는 그보다 낮다.
...
| 항목 | 내용 |
|---|---|
| `model\` | 학습된 모델 (198MB). 정규화 통계가 안에 들어 있어 **학습 데이터는 필요 없다** |
...
- 데스크톱에 있는 옛 파일(`follower_01.json` 등)을 이름만 바꿔 복사하지 말 것.
  영점 값은 파일을 읽을 때 **모터에 기록**되므로, 다른 팔의 값이면 오류 없이 조용히 어긋난 각도로 움직인다.
...
- 여기까지는 로봇도 카메라도 필요 없다. 이후는 `REAL_ROBOT_RUNBOOK.md` 7단계부터.

---

## 7. 하면 안 되는 것

- `--ensemble` 옵션. CPU 에서 루프가 30Hz 를 못 맞춘다.
